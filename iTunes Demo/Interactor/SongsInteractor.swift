...
protocol SongsInteractorProtocol {
    func fetchSongs(for keyword: String, page: Int, completion: @escaping (Result<[Song], Error>) -> Void)
}

class SongsInteractor: SongsInteractorProtocol {
    func fetchSongs(for keyword: String, page: Int = 1, completion: @escaping (Result<[Song], Error>) -> Void) {
